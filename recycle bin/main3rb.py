...
# ---------- FaceNet 特征提取类（保持原样，仅增加特征缓存相关方法）----------
class FaceNet:
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # 加载模型
        yolo_path = r'/my_yolo/runs/detect/train/weights/best.pt'
    ...
    def _capture_face(self):
        if not self.camera_thread.isRunning():
            QMessageBox.warning(self, "警告", "摄像头未开启")
            return
        QMessageBox.information(self, "提示", "请使用「上传图片」方式录入人脸，或拍照功能暂未实现")
    ...
    def _save_face_info(self):
        ...
        try:
            emb = self.face_net.get_embedding(face_img)  # 仅用于校验，实际可以不用
        except:
